...
## 目次
  ...
  - [3. 組み立て](#3-組み立て)
  - [4. 動作確認](#4-動作確認)
  - [5. ファームウェア編集](#5-ファームウェアについて)
    - [5-1. キーマップの編集](#5-1-キーマップの編集)
    - [5-2. ファームウェアの書き込み方法](#5-2ファームウェアの書き込み方法)
...
1. トラックボールをキーボード本体のL字コンスルーに差し込みます。
2. トラックボールケースを上から差し込みボトムプレート裏からねじで固定します。
3. 25mmのトラックボールをケースに差し込み組み立ては完成です。
4. それぞれのマイコンのリセットボタンを1回押します。
5. PCでbluetoothデバイスとして「moNa」を選択します。
